...
这类排序算法的本质其实是通过**两两元素间的交换**来最终实现排序。理论已经证明了基于元素交换的排序算法存在速度上限 $\Theta(nln(n))$ ，因此我们无法尝试再去在元素交换的基础上去提高速度。

但是我们对于速度的追求是永不停止的！因此能不能换种方法继续提高速度？
...
但是这样还是会有 n 的空间被浪费，那么此时我们只需要将队列改为**双端队列（Deque）** 即可实现通过一个队列放置0和1的队列。![[Pasted image 20251110222302.png]]