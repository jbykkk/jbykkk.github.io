...
![](../resources/二分搜索树、AVL树/最简单情况1.png) 

3的左子树和右子树的高度差是2 $\geq$ 1，不是平衡的，不平衡现象发生在节点3。
修正的方法也很简单，只需要将2这个中间大的元素移动为根节点即可，这样能满足左右节点的大小要求。

![](../resources/二分搜索树、AVL树/最简单情况1修正.png)

这样调整完根节点之后这个树就重新变为一个平衡的树了。

- 第二种最简单情况：

![](../resources/二分搜索树、AVL树/最简单情况2.png)

**但是**如果我们仍旧采用第一种情况的解决办法，那么1的左子树就是2，但是2是比1大的，不符合二分搜索树对于左右节点大小的要求。

此时的修正方法是这样的：

![](../resources/二分搜索树、AVL树/最简单情况2修正.png)
...
- 判断这个不平衡的类型是什么样的，具体有：
	- `LL`（*P134* ）
	- ![情况1](resources/二分搜索树、AVL树/情况1.png)
	- `LR`（*P144* ）
	- ![情况2](resources/二分搜索树、AVL树/情况2.png)
- 修复（**可能需要多次**）
...
![例子1](resources/二分搜索树、AVL树/1例子.png)

`LR` ：高度发生不平衡的节点是21

![例子2](resources/二分搜索树、AVL树/2例子.png)

至于 `RR` 和 `RL` 另外两种，只是上面两种的镜像形式，操作都是一样的。
![其他两种](resources/二分搜索树、AVL树/其他两种.png)


不平衡的情况通常发生在 `insert` 和 `erase` 操作时，因此若在一个平衡的树中进行插入则必须要判断并修复；一般来说，`insert` 至少都要一次修复，而`erase`可能需要 $O(h)$ 次修复。

`insert` 和 `erase` 的演示  *P156-P203* 
