...
- 对于没有连接到当前MST的该MST的邻居，选择一个和当前MST距离最近（$e_1$）的一个顶点（$v_2$）进行连接；
- 直到形成一个有着 $n-1$ 个边和 $n$ 个点的MST；
...
在上面的这个过程中，我们需要不断地遍历这个列表来不断更新当前MST的全部邻居并找到距离最近的那个顶点然后连接。

时间复杂度分析（有点难）：

需要遍历 $|V|-1$ 次，每次找到最近的顶点。