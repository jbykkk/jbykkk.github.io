...
- 使用邻接表，直接给出结论：$O(\|V\|ln(\|V\|) + \|E\|ln(\|V\|))=O(\|E\|ln(\|V\|))$ 。
...
// 将权重放在前，因为优先队列默认按 pair 的第一个元素排序
...
public:
    ...
    void primMST() {
        ...
        // --- 初始化起始点 ---（虚拟节点？那下一个怎么办，这个虚拟节点的邻居是谁？堆里面现在有谁）
        int src = 0; // 从顶点 0 开始（可以任意选择）
        pq.push({0, src}); // 将起点入队，权重设为 0
        key[src] = 0;      // 起点到 MST 的距离为 0

        int totalWeight = 0; // 记录 MST 总权重
        int edgeCount = 0; // 记录加入的边的数量
        ...
        while (!pq.empty() && edgeCount < V) {
            ...
            // 其实可以不要这步，因为循环已经限制了次数，不会让同一节点的多余边产生影响
            if (inMST[u]) continue;
...
但是通过堆来寻找最小的边时，时间花费出现在 `pq.pop();` ，需要 $O(ln(\|E\|))$ 的时间。而由于STL中优先队列的限制，我们无法将旧的边权重更新或删除，采用的是只增不减的方法，所以可能会有多余的边堆积，因此 $\|E\|=O(\|V\|^2)$ ，那么就是 $O(ln(\|V\|))$ 的时间。
在主循环中，这部分花费 $O(\|V\|ln(\|V\|))$ 的时间。

后面检查新加入节点的邻居节点时，时间花费出现在 `if` 判断中 `pq.push({key[v], v});` 在主循环中，最坏可能每次都要将未被访问的邻居边push进去，也就是 $\|E\|$ ， 每一次push同样需要 $O(ln(\|V\|))$ 的时间。总共就是 $O(\|E\|ln(\|V\|))$ 。

因为在 $\|E\|$ 最小也是  $\|V\|-1$ ，所以 $\|E\|$ 是更大的，
即最终：$O(\|V\|ln(\|V\|) + \|E\|ln(\|V\|))=O(\|E\|ln(\|V\|))$ 。

这里的判断不像前面一样，先在主循环内考虑各自操作，然后相加再乘上主循环的次数。而是根据各自在主循环中的操作，最后直接相加。