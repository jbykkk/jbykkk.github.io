...
- 如果仍有未被访问的顶点，继续迭代连接：
	...
	- 将**这个邻居**标记为已被访问（连入当前MST）；
	- 对于目前MST的所有邻居，如果现在连通边的距离是更小的，那么
		- 将邻居顶点到MST的距离更新为现在更近的这个距离；
		- 将**这个邻居**设为其邻居的parent；
...
1、因为最小生成树有 $|V|-1$ 条边，需要遍历 $|V|-1$ 次才能构建，$\Theta(V)$ 的时间；

- 如果使用的是邻接矩阵

2、为了找到距离最小的未被访问的邻居，需要遍历整个表，花费 $\Theta(V)$ 的时间；

3、每当我们找到一个符合要求的邻居，将其连入后还需要考虑他的全部邻居，因为要检查是否需要更新新邻居到当前MST的最小distance。